...
def renormalize_per_sample(data_feature, data_attribute, data_feature_outputs,
    ...
    data_attribute = data_attribute[:, 0: attr_dim]
    #data_attribute = np.concatenate(
        #[data_attribute, data_attribute], axis=0)
    #data_attribute = data_attribute[100:]
    #data_attribute = np.delete(data_attribute, 1, 0)
    #data_attribute = np.stack(data_attribute, axis=1)
...
def normalize_per_sample(data_feature, data_attribute, data_feature_outputs,
    ...
    data_attribute = np.concatenate(
        [data_attribute, data_attribute], axis=0)
    #data_attribute=data_attribute.reshape(-1,2)
    #data_attribute = np.stack(data_attribute, axis=1)
    additional_attribute = np.stack(additional_attribute, axis=1)
    additional_attribute=additional_attribute.reshape(-1,1)
    data_attribute=data_attribute.reshape(-1,1)
    #print(data_attribute.shape)
    #print(additional_attribute.shape)
    #sys.exit()
    #additional_attribute = additional_attribute[100:] 
    data_attribute = np.concatenate(
